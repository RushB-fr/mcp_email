...
export function buildMailAccountSchema(dict: Dictionary) {
  return z.object({
    ...
    imapHost: z.string().trim().min(1, dict.validation.imapHostRequired).default("imap.mail.ovh.net"),
    imapPort: z.coerce.number().int().min(1).max(65535).default(993),
    imapSecure: z.coerce.boolean().default(true),
    smtpHost: z.string().trim().min(1, dict.validation.smtpHostRequired).default("smtp.mail.ovh.net"),
