...
- **Clear connection diagnostics** — an IMAP/SMTP auth failure tells you plainly that the stored password is probably stale rather than a generic error, and points you straight to Réglages to update it.
...
| `DEFAULT_IMAP_HOST` / `DEFAULT_SMTP_HOST` | Optional | Pre-fills the host fields when attaching a new mailbox — purely a UI convenience, blank if unset |
...
```bash
npm install
npm run prisma:generate   # no postinstall hook - the Prisma client isn't generated by npm install alone
docker compose up -d db   # just Postgres, if you don't already have one reachable
